...
- [ ] 2.1 Criar `PortalCopa26.sln` na raiz de `PRD/src/` e verificar que `dotnet sln
  list` mostra a solução vazia
- [ ] 2.2 Criar `src/PortalCopa26.Domain` (classlib, net10.0, sem pacotes NuGet) e
  adicionar à solução; verificar que `dotnet build` do projeto isolado passa
- [ ] 2.3 Criar `src/PortalCopa26.Application` (classlib) referenciando `Domain`;
  verificar com `dotnet build`
- [ ] 2.4 Criar `src/PortalCopa26.Infrastructure` (classlib) referenciando
  `Application` e adicionar o pacote `Microsoft.EntityFrameworkCore.Sqlite`;
  verificar com `dotnet build`
- [ ] 2.5 Criar `src/PortalCopa26.Web` com `dotnet new blazor --interactivity Server`
...
- [ ] 2.6 Criar `tests/PortalCopa26.Domain.Tests` (xUnit) referenciando `Domain` e
  adicionar os pacotes `xunit` e `FluentAssertions`; verificar que `dotnet test`
  roda (mesmo sem testes ainda) sem erro
- [ ] 2.7 Confirmar que `dotnet build` na raiz da solução builda os 5 projetos sem
  avisos de referência circular ou de camada invertida (nenhum projeto downstream
  referenciado por um upstream)

## 3. Entidades de domínio

- [ ] 3.1 Implementar a entidade `Selecao` em `PortalCopa26.Domain` (código FIFA de 3
...
- [ ] 3.2 Implementar a entidade `Jogo` (fase, grupo opcional, mandante/visitante ou
...
- [ ] 3.3 Implementar `DomainException` como base das exceções de domínio; verificar
  que ela é usada pelas tarefas 3.1 e 3.2

## 4. Motor de classificação de grupos (capability `classificacao-grupos`)

- [ ] 4.1 Implementar o cálculo da tabela de um grupo (pontos, V/E/D, gols
...
- [ ] 4.2 Implementar a ordenação por desempate RN-01 (pontos → saldo → gols
...
- [ ] 4.3 Implementar a seleção dos 8 melhores terceiros colocados entre os 12
...
- [ ] 4.4 Implementar a zona de classificação de uma seleção (classificado /
...
- [ ] 5.1 Implementar a resolução de um confronto por placar no tempo normal,
...
- [ ] 5.2 Implementar a decisão por pênaltis em caso de empate no tempo normal (RN-03,
  ...
  ainda informados"
- [ ] 5.3 Implementar a propagação automática do vencedor/perdedor para o confronto
...
- [ ] 6.1 Mapear os 16 testes de `tools/testar.mjs` (`npm run teste`) para os `[Fact
  (DisplayName = "...")]`/`[Theory]` equivalentes em `PortalCopa26.Domain.Tests`, um a
  um, com nomes em português descrevendo a regra
- [ ] 6.2 Rodar `dotnet test` e verificar que todos os testes de domínio passam,
  cobrindo o mesmo conjunto de regras que os 16 testes do protótipo
- [ ] 6.3 Rodar `npm run teste` na raiz do repositório e confirmar que o protótipo
  continua com os 16 testes verdes (garantindo que nada em `src/` foi alterado)
- [ ] 6.4 Rodar `dotnet build` na raiz da solução e `dotnet run --project
